  ...
  : q_ptr(&object)
{
  ...
  //this->SavedCurrentViewArrangement = vtkMRMLLayoutNode::CjyxLayoutNone;
}
...
void qMRMLLayoutManagerPrivate::updateLayoutInternal()
{
  ...
  int layout = this->MRMLLayoutNode ?
    ...
    vtkMRMLLayoutNode::CjyxLayoutNone;

  if (layout == vtkMRMLLayoutNode::CjyxLayoutCustomView)
    {
    return;
    }
...
int qMRMLLayoutManager::layout()const
{
  ...
  return d->MRMLLayoutNode ?
    d->MRMLLayoutNode->GetViewArrangement() : vtkMRMLLayoutNode::CjyxLayoutNone;
}
...
void qMRMLLayoutManager::setLayout(int layout)
{
  ...
  if (d->MRMLLayoutNode)
    ...
    if (!d->MRMLLayoutNode->IsLayoutDescription(layout))
      {
      layout = vtkMRMLLayoutNode::CjyxLayoutConventionalView;
      }
