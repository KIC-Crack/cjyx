...
int qMRMLLayoutManagerTest2(int argc, char * argv[] )
{
...

  {
    ...
    if (!checkViewArrangement(__LINE__, layoutManager, layoutNode, vtkMRMLLayoutNode::CjyxLayoutInitialView))
      {
      return EXIT_FAILURE;
      }
  ...
  vtkMRMLLayoutNode* layoutNode = nullptr;
  {
    ...
    newLayoutNode->SetViewArrangement(vtkMRMLLayoutNode::CjyxLayoutOneUpRedSliceView);
  ...
  if (!checkViewArrangement(__LINE__, layoutManager, layoutNode, vtkMRMLLayoutNode::CjyxLayoutOneUpRedSliceView))
    {
    return EXIT_FAILURE;
    }

  layoutNode->SetViewArrangement(vtkMRMLLayoutNode::CjyxLayoutOneUpGreenSliceView);
  if (!checkViewArrangement(__LINE__, layoutManager, layoutNode, vtkMRMLLayoutNode::CjyxLayoutOneUpGreenSliceView))
    {
    return EXIT_FAILURE;
    }

  layoutManager->setLayout(vtkMRMLLayoutNode::CjyxLayoutCompareView);
  if (!checkViewArrangement(__LINE__, layoutManager, layoutNode, vtkMRMLLayoutNode::CjyxLayoutCompareView))
    {
    return EXIT_FAILURE;
    }
  ...
  scene->StartState(vtkMRMLScene::CloseState);
  layoutNode->SetViewArrangement(vtkMRMLLayoutNode::CjyxLayoutOneUpRedSliceView);
  scene->EndState(vtkMRMLScene::CloseState);
  ...
  layoutNode->SetViewArrangement(vtkMRMLLayoutNode::CjyxLayoutNone);
