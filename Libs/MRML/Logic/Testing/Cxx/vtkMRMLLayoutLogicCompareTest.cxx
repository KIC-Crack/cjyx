...
bool TestSetCjyxLayoutCompareGridView()
{
  ...
  layoutNode->SetViewArrangement(vtkMRMLLayoutNode::CjyxLayoutCompareGridView);
  if (layoutNode->GetViewArrangement() != vtkMRMLLayoutNode::CjyxLayoutCompareGridView)
    {
...
bool TestSetCjyxLayoutCompareGridViewEvents()
{
  ...
  layoutNode->SetViewArrangement(vtkMRMLLayoutNode::CjyxLayoutFourUpView);
  if (spy->GetTotalNumberOfEvents() != 1 ||
      spy->GetNumberOfEvents(vtkCommand::ModifiedEvent) != 1)
    {
    std::cout << __LINE__ << ": SetViewArrangement failed. "
