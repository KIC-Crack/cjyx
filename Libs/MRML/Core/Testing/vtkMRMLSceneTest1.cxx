...
int vtkMRMLSceneTest1(int , char * [] )
{
  ...
  int revision = -1;
  // Old-style
  CHECK_BOOL(vtkMRMLScene::ParseVersion("Cjyx4.5.6", applicationName, major, minor, patch, revision), true);
  CHECK_STD_STRING(applicationName, "Cjyx");
  CHECK_INT(major, 4);
  CHECK_INT(minor, 5);
  CHECK_INT(patch, 6);
  // Invalid
  CHECK_BOOL(vtkMRMLScene::ParseVersion("Cjyx4.5.", applicationName, major, minor, patch, revision), false);
