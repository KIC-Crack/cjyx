...
def childWidgetVariables(widget):
    ...
    Example::
      ...
      self.ui.inputSelector.setMRMLScene(cjyx.mrmlScene)
      self.ui.outputSelector.setMRMLScene(cjyx.mrmlScene)
...
def setSliceViewerLayers(background='keep-current', foreground='keep-current', label='keep-current',
    ...
    def _nodeID(nodeOrID):
        nodeID = nodeOrID
        if isinstance(nodeOrID, cjyx.vtkMRMLNode):
            nodeID = nodeOrID.GetID()
        return nodeID

    num = cjyx.mrmlScene.GetNumberOfNodesByClass('vtkMRMLSliceCompositeNode')
    for i in range(num):
        sliceViewer = cjyx.mrmlScene.GetNthNodeByClass(i, 'vtkMRMLSliceCompositeNode')
    ...
    if rotateToVolumePlane:
        if background != 'keep-current':
            volumeNode = cjyx.mrmlScene.GetNodeByID(_nodeID(background))
        elif foreground != 'keep-current':
            volumeNode = cjyx.mrmlScene.GetNodeByID(_nodeID(foreground))
        elif label != 'keep-current':
            volumeNode = cjyx.mrmlScene.GetNodeByID(_nodeID(label))
        else:
            volumeNode = None
        if volumeNode:
...
def loadNodeFromFile(filename, filetype, properties={}, returnNode=False):
    ...
    """
    from cjyx import app, vtkMRMLMessageCollection
...
def exportNode(node, filename, properties={}, world=False):
    ...
    """
    from cjyx import app, vtkDataFileFormatHelper, vtkMRMLMessageCollection
    nodeIDs = [node.GetID()]
    fileNames = [filename]
...
def getNode(pattern="*", index=0, scene=None):
    ...
    associated with ``cjyx.mrmlScene``.
...
def getNodesByClass(className, scene=None):
    ...
    if scene is None:
        scene = cjyx.mrmlScene
    nodes = cjyx.mrmlScene.GetNodesByClass(className)
    nodes.UnRegister(cjyx.mrmlScene)
    nodeList = []
    nodes.InitTraversal()
...
def array(pattern="", index=0):
    ...
    import cjyx
    if isinstance(node, cjyx.vtkMRMLVolumeNode):
        return arrayFromVolume(node)
    elif isinstance(node, cjyx.vtkMRMLModelNode):
        return arrayFromModelPoints(node)
    elif isinstance(node, cjyx.vtkMRMLGridTransformNode):
        return arrayFromGridTransform(node)
    elif isinstance(node, cjyx.vtkMRMLMarkupsNode):
        return arrayFromMarkupsControlPoints(node)
    elif isinstance(node, cjyx.vtkMRMLTransformNode):
...
def arrayFromSegmentBinaryLabelmap(segmentationNode, segmentId, referenceVolumeNode=None):
    ...
    if not referenceVolumeNode:
        referenceVolumeNode = segmentationNode.GetNodeReference(cjyx.vtkMRMLSegmentationNode.GetReferenceImageGeometryReferenceRole())
        if not referenceVolumeNode:
    ...
    labelmapVolumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLLabelMapVolumeNode", "__temp__")
    try:
...
def updateVolumeFromArray(volumeNode, narray):
    ...
    volumeNode.Modified()
    volumeNode.InvokeEvent(cjyx.vtkMRMLVolumeNode.ImageDataModifiedEvent, volumeNode)
...
def addVolumeFromArray(narray, ijkToRAS=None, name=None, nodeClassName=None):
    ...
    volumeNode = cjyx.mrmlScene.AddNewNodeByClass(nodeClassName, name)
...
def updateTableFromArray(tableNode, narrays, columnNames=None):
    ...
    Example::
      ...
      tableNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLTableNode")
...
def plot(narray, xColumnIndex=-1, columnNames=None, title=None, show=True, nodes=None):
    ...
    .. code-block:: python
      ...
      chartNode.GetNthPlotSeriesNode(0).SetPlotType(cjyx.vtkMRMLPlotSeriesNode.PlotTypeScatterBar)
    ...
    if chartNode is None:
        chartNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLPlotChartNode")
    if title is not None:
    ...
    for columnIndex in range(numberOfColumns):
        ...
        if len(seriesNodes) > seriesIndex:
            ...
            seriesNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLPlotSeriesNode")
