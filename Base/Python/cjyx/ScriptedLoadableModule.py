...
class ScriptedLoadableModuleWidget:
    def __init__(self, parent=None):
        ...
        if not parent:
            self.parent = cjyx.qMRMLWidget()
            self.parent.setLayout(qt.QVBoxLayout())
            self.parent.setMRMLScene(cjyx.mrmlScene)
        else:
            self.parent = parent
...
class ScriptedLoadableModuleLogic:
    ...
    def getParameterNode(self):
        ...
        if self.isSingletonParameterNode:
            parameterNode = cjyx.mrmlScene.GetSingletonNode(self.moduleName, "vtkMRMLScriptedModuleNode")
            if parameterNode:
            ...
            numberOfScriptedModuleNodes = cjyx.mrmlScene.GetNumberOfNodesByClass("vtkMRMLScriptedModuleNode")
            for nodeIndex in range(numberOfScriptedModuleNodes):
                parameterNode = cjyx.mrmlScene.GetNthNodeByClass(nodeIndex, "vtkMRMLScriptedModuleNode")
                if parameterNode.GetAttribute("ModuleName") == self.moduleName:
                    return parameterNode
        # no parameter node was found for this module, therefore we add a new one now
        parameterNode = cjyx.mrmlScene.AddNode(self.createParameterNode())
        return parameterNode
    ...
    def createParameterNode(self):
        ...
        """
        if cjyx.mrmlScene is None:
            return

        node = cjyx.mrmlScene.CreateNodeByClass("vtkMRMLScriptedModuleNode")
        ...
        node.SetName(cjyx.mrmlScene.GenerateUniqueName(self.moduleName))
        return node
...
class ScriptedLoadableModuleTest(unittest.TestCase):
    ...
    def takeScreenshot(self, name, description, type=-1):
        ...
        :param type: which viewport to capture. If not specified then captures the entire window.
            Valid values: cjyx.qMRMLScreenShotDialog.FullLayout,
            cjyx.qMRMLScreenShotDialog.ThreeD, cjyx.qMRMLScreenShotDialog.Red,
            cjyx.qMRMLScreenShotDialog.Yellow, cjyx.qMRMLScreenShotDialog.Green.
        """
        ...
        widget = 0
        if type == cjyx.qMRMLScreenShotDialog.FullLayout:
            # full layout
            widget = lm.viewport()
        elif type == cjyx.qMRMLScreenShotDialog.ThreeD:
            # just the 3D window
            widget = lm.threeDWidget(0).threeDView()
        elif type == cjyx.qMRMLScreenShotDialog.Red:
            # red slice window
            widget = lm.sliceWidget("Red")
        elif type == cjyx.qMRMLScreenShotDialog.Yellow:
            # yellow slice window
            widget = lm.sliceWidget("Yellow")
        elif type == cjyx.qMRMLScreenShotDialog.Green:
            # green slice window
            ...
            type = cjyx.qMRMLScreenShotDialog.FullLayout

        # grab and convert to vtk image data
        qimage = ctk.ctkWidgetsUtils.grabWidget(widget)
        imageData = vtk.vtkImageData()
        cjyx.qMRMLUtils().qImageToVtkImageData(qimage, imageData)
