...
class CjyxUtilGetNodeTest(unittest.TestCase):
    ...
    def test_getFirstNodeByName(self):
        self.assertEqual(cjyx.util.getFirstNodeByName("Volume", 'vtkMRMLScalarVolumeNode').GetName(), "Volume1")

    def test_getNode(self):

        # Test handling of requesting non-existing node
        with self.assertRaises(cjyx.util.MRMLNodeNotFoundException):
            cjyx.util.getNode("")
        with self.assertRaises(cjyx.util.MRMLNodeNotFoundException):
