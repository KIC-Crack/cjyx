                                                   ...
                                                   vtkMRMLScene* mrmlScene, const QString& nodeID)
{
  ...
  if (!nodeID.isEmpty())
    {
    object = qCjyxSaveDataDialogPrivate::getNodeByID(nodeID.toUtf8().data(), mrmlScene);
    }
  if (!object)
    {
...
//-----------------------------------------------------------------------------
void qCjyxSaveDataDialogPrivate::setMRMLScene(vtkMRMLScene* scene)
{
...
//-----------------------------------------------------------------------------
vtkMRMLScene* qCjyxSaveDataDialogPrivate::mrmlScene()const
{
...
//-----------------------------------------------------------------------------
void qCjyxSaveDataDialogPrivate::populateNode(vtkMRMLNode* node)
{
...
//-----------------------------------------------------------------------------
QFileInfo qCjyxSaveDataDialogPrivate::nodeFileInfo(vtkMRMLStorableNode* node)
{
...
//-----------------------------------------------------------------------------
QTableWidgetItem* qCjyxSaveDataDialogPrivate::createNodeNameItem(vtkMRMLStorableNode* node)
{
...
//-----------------------------------------------------------------------------
QTableWidgetItem* qCjyxSaveDataDialogPrivate::createNodeTypeItem(vtkMRMLStorableNode* node)
{
...
//-----------------------------------------------------------------------------
QWidget* qCjyxSaveDataDialogPrivate::createFileFormatsWidget(vtkMRMLStorableNode* node, QFileInfo& fileInfo)
{
  vtkMRMLStorageNode* snode = node->GetStorageNode();
  Q_ASSERT(snode);
...
::createFileNameItem(const QFileInfo& fileInfo, const QString& extension, const QString& nodeID)
{
  QTableWidgetItem* fileNameItem = new QTableWidgetItem(
    qCjyxFileNameItemDelegate::forceFileNameExtension(fileInfo.fileName(), extension, this->mrmlScene(), nodeID));
...
//-----------------------------------------------------------------------------
vtkMRMLNode* qCjyxSaveDataDialogPrivate::getNodeByID(char *id)const
{
  return qCjyxSaveDataDialogPrivate::getNodeByID(id, this->MRMLScene);
}

//-----------------------------------------------------------------------------
vtkMRMLNode* qCjyxSaveDataDialogPrivate::getNodeByID(char *id, vtkMRMLScene* scene)
{
...
void qCjyxSaveDataDialogPrivate::formatChanged(int row)
{
  ...
  fileNameItem->setText(
        qCjyxFileNameItemDelegate::forceFileNameExtension(fileNameItem->text(), extension, this->MRMLScene, nodeID));
...
//-----------------------------------------------------------------------------
void qCjyxSaveDataDialogPrivate::updateStatusIconFromMessageCollection(int row, vtkMRMLMessageCollection* userMessages, bool success)
{
  QString messagesStr;
...
void qCjyxSaveDataDialogPrivate::onItemChanged(QTableWidgetItem* widgetItem)
{
  ...
  if (!nodeID.isEmpty())
    {
    objectToSave = qCjyxSaveDataDialogPrivate::getNodeByID(nodeID.toUtf8().data(), mrmlScene);
    if (!objectToSave)
      {
      qCritical() << Q_FUNC_INFO << " failed: node not found by ID " << qPrintable(nodeID);
      return;
...
bool qCjyxSaveDataDialog::exec(const qCjyxIO::IOProperties& readerProperties)
{
  ...
  d->setMRMLScene(qCjyxCoreApplication::application()->mrmlScene());
