...
class CjyxMRBSaveRestoreCheckPaths(ScriptedLoadableModuleTest):
    ...
    def checkSceneViewFileNames(self, scene):
        ...
        for n in range(numberOfSceneViews):
            sceneViewNode = cjyx.mrmlScene.GetNthNodeByClass(n, 'vtkMRMLSceneViewNode')
    ...
    def test_CjyxMRBSaveRestoreCheckPaths(self):
        ...
        #
        self.checkAllFileNames(cjyx.mrmlScene)
        ...
        #
        tempDir = cjyx.util.tempDirectory('__mrml__')
        cjyx.util.delayDisplay('Temp dir = %s ' % tempDir)
        mrmlFilePath = tempDir + '/CjyxMRBSaveRestoreCheckPath.mrml'
        cjyx.mrmlScene.SetURL(mrmlFilePath)
        cjyx.util.delayDisplay(f'Saving mrml file to {mrmlFilePath}, current url of scene is {cjyx.mrmlScene.GetURL()}')
        # saveScene just writes out the .mrml file
        self.assertTrue(ioManager.saveScene(mrmlFilePath, screenShot))
        cjyx.util.delayDisplay(f'Finished saving mrml file {mrmlFilePath}, mrml url is now {cjyx.mrmlScene.GetURL()}\n\n\n')
        cjyx.util.delayDisplay('mrml root dir = %s' % cjyx.mrmlScene.GetRootDirectory())
