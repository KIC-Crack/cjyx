...
class ScenePerformanceWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        widget.setMRMLScene(cjyx.mrmlScene)
        # self.MRMLNodeComboBox.setMRMLScene(cjyx.mrmlScene)
...
class ScenePerformanceTest(ScriptedLoadableModuleTest):
    ...
    def restoreSceneView(self, sceneViewIndex):
        node = cjyx.mrmlScene.GetNthNodeByClass(sceneViewIndex, 'vtkMRMLSceneViewNode')
    ...
    def addNodeByID(self, nodeID):
        node = cjyx.mrmlScene.GetNodeByID(nodeID)
    ...
    def addNode(self, node):
        ...
        for x in range(self.Repeat):
            ...
            newNode.Copy(node)
            logic.startTiming()
            cjyx.mrmlScene.AddNode(newNode)
            time = logic.stopTiming()
