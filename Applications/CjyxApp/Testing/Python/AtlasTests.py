...
class AtlasTestsTest(ScriptedLoadableModuleTest):
    ...
    def perform_AtlasTest(self, downloads, testVolumePattern):
        ...
        numModelHierarchies = cjyx.mrmlScene.GetNumberOfNodesByClass("vtkMRMLModelHierarchyNode")
        # get the number that we'll be manipulating
        numModelHierarchiesToManipulate = 0
        for h in range(numModelHierarchies):
            mh = cjyx.mrmlScene.GetNthNodeByClass(h, "vtkMRMLModelHierarchyNode")
        ...
        numSceneViews = cjyx.mrmlScene.GetNumberOfNodesByClass("vtkMRMLSceneViewNode")
        for s in range(numSceneViews):
            sv = cjyx.mrmlScene.GetNthNodeByClass(s, "vtkMRMLSceneViewNode")
