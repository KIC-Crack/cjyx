...
if modelNode.GetParentTransformNode():
  ...
  cjyx.vtkMRMLTransformNode.GetTransformBetweenNodes(modelNode.GetParentTransformNode(), None, transformModelToWorld)
...
resultTableNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLTableNode", "Points from surface distance")
...
cjyx.app.layoutManager().setLayout(cjyx.vtkMRMLLayoutNode.CjyxLayoutFourUpTableView)
...
if not pointListNode:
  pointListNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsFiducialNode","F")
...
pointListNodeObserverTag = markupsNode.AddObserver(cjyx.vtkMRMLMarkupsFiducialNode.PointModifiedEvent, onPointsModified)
...
for modelIndex, modelFile in enumerate(modelFiles):
  ...
  viewNode = cjyx.mrmlScene.GetSingletonNode(name, "vtkMRMLViewNode")
...
referenceVolumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode")
...
outputLabelmapVolumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLLabelMapVolumeNode")
