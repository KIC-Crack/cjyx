...
```python
markupsNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsCurveNode")
...
curveNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsCurveNode")
...
markupsNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsCurveNode")
w.setCurrentNode(cjyx.mrmlScene.GetNodeByID(markupsNode.GetID()))
...
```python
selectionNode = cjyx.mrmlScene.GetNodeByID("vtkMRMLSelectionNodeSingleton")
selectionNode.SetReferenceActivePlaceNodeClassName("vtkMRMLMarkupsFiducialNode")
interactionNode = cjyx.mrmlScene.GetNodeByID("vtkMRMLInteractionNodeSingleton")
placeModePersistence = 1
interactionNode.SetPlaceModePersistence(placeModePersistence)
# mode 1 is Place, can also be accessed via cjyx.vtkMRMLInteractionNode().Place
...
pointListNode.AddObserver(cjyx.vtkMRMLMarkupsNode.PointModifiedEvent, UpdateSphere, 2)
```
...
roiNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsROINode")

cropVolumeParameters = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLCropVolumeParametersNode")
...
cjyx.mrmlScene.RemoveNode(cropVolumeParameters)
```
...
planeNode = cjyx.mrmlScene.AddNewNodeByClass('vtkMRMLMarkupsPlaneNode')
...
def ShowAngle(unused1=None, unused2=None):
  ...
  for planeNodeName in planeNodeNames:
    planeNode = cjyx.util.getFirstNodeByClassByName("vtkMRMLMarkupsPlaneNode", planeNodeName)
...
for planeNodeName in planeNodeNames:
  planeNode = cjyx.util.getFirstNodeByClassByName("vtkMRMLMarkupsPlaneNode", planeNodeName)
  planeNode.AddObserver(cjyx.vtkMRMLMarkupsPlaneNode.PointModifiedEvent, ShowAngle)
...
def ShowAngle(unused1=None, unused2=None):
  ...
  for lineNodeName in lineNodeNames:
    lineNode = cjyx.util.getFirstNodeByClassByName("vtkMRMLMarkupsLineNode", lineNodeName)
...
for lineNodeName in lineNodeNames:
  lineNode = cjyx.util.getFirstNodeByClassByName("vtkMRMLMarkupsLineNode", lineNodeName)
  lineNode.AddObserver(cjyx.vtkMRMLMarkupsLineNode.PointModifiedEvent, ShowAngle)
...
projectedLineNode = cjyx.mrmlScene.AddNewNodeByClass(lineNode.GetClassName(), lineNode.GetName()+" projected")
...
pointListObservation = [pointListNode, pointListNode.AddObserver(cjyx.vtkMRMLMarkupsNode.PointModifiedEvent, UpdateSlicePlane, 2)]
```
...
placeWidget.setMRMLScene(cjyx.mrmlScene)
...
def onMarkupChanged(caller,event):
  ...
  if movingMarkupIndex >= 0:
    ...
    isPreview = markupsNode.GetNthControlPointPositionStatus(movingMarkupIndex) == cjyx.vtkMRMLMarkupsNode.PositionPreview
    if isPreview:
...
pointListNode.AddObserver(cjyx.vtkMRMLMarkupsNode.PointModifiedEvent, onMarkupChanged)
pointListNode.AddObserver(cjyx.vtkMRMLMarkupsNode.PointStartInteractionEvent, onMarkupStartInteraction)
pointListNode.AddObserver(cjyx.vtkMRMLMarkupsNode.PointEndInteractionEvent, onMarkupEndInteraction)
```
...
pointListNode = cjyx.mrmlScene.GetFirstNodeByName("F")
...
def createMeasurements():
  for nodeName in ['A', 'B', 'C', 'D']:
    lineNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsLineNode", nodeName)
...
def copyLineMeasurementsToClipboard():
  ...
  for lineNode in lineNodes:
    ...
    try:
      volumeNode = cjyx.mrmlScene.GetNodeByID(lineNode.GetNthControlPointAssociatedNodeID(0))
...
markupsDisplayNodes = cjyx.util.getNodesByClass("vtkMRMLMarkupsDisplayNode")
