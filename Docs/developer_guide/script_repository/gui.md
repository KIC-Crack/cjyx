...
```python
volumeNode = cjyx.mrmlScene.GetFirstNodeByClass("vtkMRMLScalarVolumeNode")
...
- `cjyx.mrmlScene.GetNodeByID()` is more appropriate when a module needs to access a MRML node:
  - its behavior is more predictable: it only accepts node ID as input. `cjyx.mrmlScene.GetFirstNodeByName()` can be used to get a node by its name, but since multiple nodes in the scene can have the same name, it is not recommended to keep reference to a node by its name. Since node IDs may change when a scene is saved and reloaded, node ID should not be stored persistently, but [node references](mrml_overview.md#mrml-node-references) must be used instead
...
```python
defaultModelStorageNode = cjyx.vtkMRMLModelStorageNode()
defaultModelStorageNode.SetDefaultWriteFileExtension("ply")
cjyx.mrmlScene.AddDefaultNode(defaultModelStorageNode)
```
...
existingModelStorageNodes = cjyx.util.getNodesByClass("vtkMRMLModelStorageNode")
for modelStorageNode in existingModelStorageNodes:
  cjyx.mrmlScene.RemoveNode(modelStorageNode)
```
...
volumeNodes = cjyx.util.getNodesByClass("vtkMRMLScalarVolumeNode")
...
defaultVolumeStorageNode = cjyx.vtkMRMLVolumeArchetypeStorageNode()
defaultVolumeStorageNode.SetUseCompression(0)
cjyx.mrmlScene.AddDefaultNode(defaultVolumeStorageNode)
...
```python
#set the default volume storage to not compress by default
defaultVolumeStorageNode = cjyx.vtkMRMLSegmentationStorageNode()
defaultVolumeStorageNode.SetUseCompression(0)
cjyx.mrmlScene.AddDefaultNode(defaultVolumeStorageNode)
...
```python
viewNodes = cjyx.util.getNodesByClass("vtkMRMLAbstractViewNode")
for viewNode in viewNodes:
  viewNode.SetOrientationMarkerType(cjyx.vtkMRMLAbstractViewNode.OrientationMarkerTypeAxes)
```
...
```python
labels = ["x", "X", "y", "Y", "z", "Z"]
viewNode = cjyx.app.layoutManager().threeDWidget(0).mrmlViewNode()
# for slice view:
# viewNode = cjyx.app.layoutManager().sliceWidget("Red").mrmlSliceNode()
...
crosshairNode.AddObserver(cjyx.vtkMRMLCrosshairNode.CursorPositionModifiedEvent, onMouseMoved)
```
...
cjyx.vtkMRMLSliceNode.JumpAllSlices(cjyx.mrmlScene, *position_RAS, cjyx.vtkMRMLSliceNode.CenteredJumpSlice)
# Make the crosshair visible
crosshairNode.SetCrosshairMode(cjyx.vtkMRMLCrosshairNode.ShowBasic)
```
...
crosshairNode=cjyx.util.getNode("Crosshair")
observationId = crosshairNode.AddObserver(cjyx.vtkMRMLCrosshairNode.CursorPositionModifiedEvent, onMouseMoved)
...
```python
invertedocean = cjyx.vtkMRMLColorTableNode()
...
cjyx.mrmlScene.AddNode(invertedocean)
```
...
# Create color node
colorNode = cjyx.mrmlScene.CreateNodeByClass("vtkMRMLProceduralColorNode")
colorNode.UnRegister(None)  # to prevent memory leaks
colorNode.SetName(cjyx.mrmlScene.GenerateUniqueName("MyColormap"))
...
cjyx.mrmlScene.AddNode(colorNode)
...
```python
sliceDisplayNodes = cjyx.util.getNodesByClass("vtkMRMLSliceDisplayNode")
...
sliceNodes = cjyx.util.getNodesByClass('vtkMRMLSliceNode')
...
```python
sliceNode = cjyx.app.layoutManager().sliceWidget("Red").mrmlSliceNode()
...
def ShowAngle(unused1=None, unused2=None):
  ...
  for sliceNodeId in sliceNodeIds:
    sliceToRAS = cjyx.mrmlScene.GetNodeByID(sliceNodeId).GetSliceToRAS()
...
for sliceNodeId in sliceNodeIds:
  cjyx.mrmlScene.GetNodeByID(sliceNodeId).AddObserver(vtk.vtkCommand.ModifiedEvent, ShowAngle)
...
sliceNodes = cjyx.util.getNodesByClass("vtkMRMLSliceNode")
sliceNodes.append(cjyx.mrmlScene.GetDefaultNodeByClass("vtkMRMLSliceNode"))
for sliceNode in sliceNodes:
...
sliceCompositeNodes = cjyx.util.getNodesByClass("vtkMRMLSliceCompositeNode")
defaultSliceCompositeNode = cjyx.mrmlScene.GetDefaultNodeByClass("vtkMRMLSliceCompositeNode")
if not defaultSliceCompositeNode:
  defaultSliceCompositeNode = cjyx.mrmlScene.CreateNodeByClass("vtkMRMLSliceCompositeNode")
  ...
  cjyx.mrmlScene.AddDefaultNode(defaultSliceCompositeNode)
...
```python
crosshair=cjyx.mrmlScene.GetFirstNodeByClass("vtkMRMLCrosshairNode")
...
viewOwnerNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLScriptedModuleNode")

# Create MRML nodes
viewLogic = cjyx.vtkMRMLSliceLogic()
viewLogic.SetMRMLScene(cjyx.mrmlScene)
...
# Create widget
viewWidget = cjyx.qMRMLSliceWidget()
viewWidget.setMRMLScene(cjyx.mrmlScene)
...
# Create MRML node
viewLogic = cjyx.vtkMRMLViewLogic()
viewLogic.SetMRMLScene(cjyx.mrmlScene)
...
# Create widget
viewWidget = cjyx.qMRMLThreeDWidget()
viewWidget.setMRMLScene(cjyx.mrmlScene)
...
```python
shortcuts = [
  ("Ctrl+b", lambda: cjyx.app.layoutManager().setLayout(cjyx.vtkMRMLLayoutNode.CjyxLayoutOneUpRedSliceView)),
  ("Ctrl+n", lambda: cjyx.app.layoutManager().setLayout(cjyx.vtkMRMLLayoutNode.CjyxLayoutOneUpYellowSliceView)),
  ("Ctrl+m", lambda: cjyx.app.layoutManager().setLayout(cjyx.vtkMRMLLayoutNode.CjyxLayoutOneUpGreenSliceView)),
  ("Ctrl+,", lambda: cjyx.app.layoutManager().setLayout(cjyx.vtkMRMLLayoutNode.CjyxLayoutFourUpView))
  ]
...
widget.SetEventTranslation(widget.WidgetStateIdle, cjyx.vtkMRMLInteractionEventData.LeftButtonClickEvent, vtk.vtkEvent.NoModifier, widget.WidgetEventSetCrosshairPosition)
widget.SetEventTranslation(widget.WidgetStateIdle, cjyx.vtkMRMLInteractionEventData.LeftButtonClickEvent, vtk.vtkEvent.NoModifier, widget.WidgetEventSetCrosshairPosition)
