...
volumeNode=cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode")
...
cjyx.mrmlScene.AddObserver(cjyx.vtkMRMLScene.NodeAddedEvent, onNodeAdded)
```
...
def showVolumeRenderingMIP(volumeNode, useSliceViewColors=True):
  ...
  for viewNode in cjyx.util.getNodesByClass("vtkMRMLViewNode"):
    viewNode.SetRaycastTechnique(cjyx.vtkMRMLViewNode.MaximumIntensityProjection)
...
clonedVolumeNode = volumesLogic.CloneVolume(cjyx.mrmlScene, sourceVolumeNode, "Cloned volume")
```
...
# Create volume node
volumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode", nodeName)
...
cjyx.vtkMRMLTransformNode.GetTransformBetweenNodes(None, volumeNode.GetParentTransformNode(), transformRasToVolumeRas)
...
cjyx.vtkMRMLTransformNode.GetTransformBetweenNodes(volumeNode.GetParentTransformNode(), None, transformVolumeRasToRas)
...
```python
cjyx.app.applicationLogic().GetInteractionNode().SetCurrentInteractionMode(cjyx.vtkMRMLInteractionNode.AdjustWindowLevel)
```
...
cjyx.mrmlScene.AddObserver(cjyx.mrmlScene.NodeAddedEvent, NoInterpolate)
```
...
outputVolumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode", "MRHeadFiltered")
...
sliceNode = cjyx.mrmlScene.GetNodeByID(sliceNodeID)
...
```python
  ...
  volumeNode = cjyx.mrmlScene.GetNodeByID('vtkMRMLScalarVolumeNode1')
...
# Read presets scene
customPresetsScene = cjyx.vtkMRMLScene()
vrPropNode = cjyx.vtkMRMLVolumePropertyNode()
