...
seg = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLSegmentationNode")
cjyx.modules.segmentations.logic().ImportLabelmapToSegmentationNode(labelmapVolumeNode, seg)
seg.CreateClosedSurfaceRepresentation()
cjyx.mrmlScene.RemoveNode(labelmapVolumeNode)
```
...
# Create segmentation
segmentationNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLSegmentationNode")
...
labelmapVolumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLLabelMapVolumeNode")
...
def exportLabelmap():
  ...
  referenceVolumeNode = cjyx.mrmlScene.GetFirstNodeByClass("vtkMRMLScalarVolumeNode")
  labelmapVolumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLLabelMapVolumeNode")
  cjyx.modules.segmentations.logic().ExportVisibleSegmentsToLabelmapNode(segmentationNode, labelmapVolumeNode, referenceVolumeNode)
  filepath = outputPath + "/" + referenceVolumeNode.GetName() + "-label.nrrd"
  cjyx.util.saveNode(labelmapVolumeNode, filepath)
  cjyx.mrmlScene.RemoveNode(labelmapVolumeNode.GetDisplayNode().GetColorNode())
  cjyx.mrmlScene.RemoveNode(labelmapVolumeNode)
...
colorTableNode = cjyx.mrmlScene.CreateNodeByClass("vtkMRMLColorTableNode")
colorTableNode.SetTypeToUser()
colorTableNode.HideFromEditorsOff()  # make the color table selectable in the GUI outside Colors module
cjyx.mrmlScene.AddNode(colorTableNode); colorTableNode.UnRegister(None)
...
labelmapVolumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLLabelMapVolumeNode")  # export to new labelmap volume
...
segmentationNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLSegmentationNode")  # import into new segmentation node
colorTableNode = ...  # created from scratch or loaded from file
...
# Set up Hollow tool
hollowModeler = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLDynamicModelerNode")
hollowModeler.SetToolName("Hollow")
hollowModeler.SetNodeReferenceID("Hollow.InputModel", modelNode.GetID())
hollowedModelNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLModelNode")  # this node will store the hollow model
...
cjyx.vtkMRMLTransformNode.GetTransformBetweenNodes(segmentationNode.GetParentTransformNode(), None, transformWorldToRas)
...
# Create segmentation
segmentationNode = cjyx.vtkMRMLSegmentationNode()
cjyx.mrmlScene.AddNode(segmentationNode)
segmentationNode.CreateDefaultDisplayNodes() # only needed for display
...
```python
segmentationNode = cjyx.mrmlScene.GetFirstNodeByClass("vtkMRMLSegmentationNode")
pointListNode = cjyx.mrmlScene.GetFirstNodeByClass("vtkMRMLMarkupsFiducialNode")
...
pointListNode.AddObserver(cjyx.vtkMRMLMarkupsPlaneNode.PointModifiedEvent, printSegmentNames)
printSegmentNames()
```
...
```python
defaultSegmentEditorNode = cjyx.vtkMRMLSegmentEditorNode()
defaultSegmentEditorNode.SetOverwriteMode(cjyx.vtkMRMLSegmentEditorNode.OverwriteNone)
cjyx.mrmlScene.AddDefaultNode(defaultSegmentEditorNode)
```
...
for segmentId in stats["SegmentIDs"]:
  ...
  roi=cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsROINode")
...
for segmentId in stats["SegmentIDs"]:
  ...
  # Create ROI
  segment = segmentationNode.GetSegmentation().GetSegment(segmentId)
  roi=cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLAnnotationROINode")
