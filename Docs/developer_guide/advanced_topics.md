...
```py
nodes = cjyx.mrmlScene.GetNodesByClass('vtkMRMLLinearTransformNode')
nodes.UnRegister(cjyx.mrmlScene) # reference count is increased by both the factory method and the python reference; unregister to keep only the python reference
# ...
```
...
```py
n = cjyx.mrmlScene.CreateNodeByClass('vtkMRMLLinearTransformNode')
cjyx.mrmlScene.AddNode(n)
n.UnRegister(cjyx.mrmlScene)
```

this should be used:

```py
n = cjyx.mrmlScene.AddNewNodeByClass('vtkMRMLLinearTransformNode')
```
