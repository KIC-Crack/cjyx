...
class SubjectHierarchyGenericSelfTestTest(ScriptedLoadableModuleTest):
    ...
    def section_SetupPathsAndNames(self):
        ...
        self.invalidItemID = cjyx.vtkMRMLSubjectHierarchyNode.GetInvalidItemID()
    ...
    def section_ClearScene(self):
        ...
        self.assertEqual(cjyx.mrmlScene.GetNumberOfNodesByClass('vtkMRMLSubjectHierarchyNode'), 1)
    ...
    def section_LoadDicomDataWitchBatchProcessing(self):
        try:
            ...
            with DICOMUtils.TemporaryDICOMDatabase(self.dicomDatabaseDir) as db:
                ...
                cjyx.mrmlScene.StartState(cjyx.vtkMRMLScene.BatchProcessState)
                ...
                cjyx.mrmlScene.EndState(cjyx.vtkMRMLScene.BatchProcessState)

            self.assertEqual(len(cjyx.util.getNodes('vtkMRMLSubjectHierarchyNode*')), 1)
    ...
    def section_SaveScene(self):
        ...
        cjyx.mrmlScene.Commit(self.genericTestSceneFileName)
    ...
    def section_AddNodeToSubjectHierarchy(self):
        ...
        volumeNodes = list(cjyx.util.getNodes('vtkMRMLScalarVolumeNode*').values())
        ...
        # Verify DICOM levels
        self.assertEqual(shNode.GetItemLevel(self.patientItemID), cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMLevelPatient())
        self.assertEqual(shNode.GetItemLevel(self.studyItemID), cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMLevelStudy())
    ...
    def section_CLI(self):
        ...
        resampledVolumeNode = cjyx.vtkMRMLScalarVolumeNode()
        resampledVolumeNode.SetName(ctVolumeNode.GetName() + '_Resampled_10x10x10mm')
        cjyx.mrmlScene.AddNode(resampledVolumeNode)
    ...
    def section_LoadScene(self):
        ...
        self.assertEqual(cjyx.mrmlScene.GetNumberOfNodesByClass('vtkMRMLScalarVolumeNode'), 4)
        self.assertEqual(cjyx.mrmlScene.GetNumberOfNodesByClass('vtkMRMLModelNode'), 4)  # Including the three slice view models
        self.assertEqual(cjyx.mrmlScene.GetNumberOfNodesByClass('vtkMRMLSubjectHierarchyNode'), 1)
    ...
    def section_AttributeFilters(self):
        ...
        self.assertEqual(cjyx.mrmlScene.GetNumberOfNodesByClass('vtkMRMLMarkupsNode'), 7)

        # Create test SH tree view
        shTreeView = cjyx.qMRMLSubjectHierarchyTreeView()
        shTreeView.setMRMLScene(cjyx.mrmlScene)
        shTreeView.show()
    ...
    def section_ComboboxFeatures(self):
        ...
        comboBox = cjyx.qMRMLSubjectHierarchyComboBox()
        comboBox.setMRMLScene(cjyx.mrmlScene)
        comboBox.show()
    ...
    def createSampleLabelmapVolumeNode(self, volumeNode, name, label, colorNode=None):
        ...
        sampleLabelmapNode = cjyx.vtkMRMLLabelMapVolumeNode()
        sampleLabelmapNode.SetName(name)
        sampleLabelmapNode = cjyx.mrmlScene.AddNode(sampleLabelmapNode)
        ...
        # Display labelmap
        labelmapVolumeDisplayNode = cjyx.vtkMRMLLabelMapVolumeDisplayNode()
        cjyx.mrmlScene.AddNode(labelmapVolumeDisplayNode)
        ...
        sampleLabelmapName = cjyx.mrmlScene.GenerateUniqueName(name)
    ...
    def createSampleModelNode(self, name, color, volumeNode=None):
        ...
        modelNode = cjyx.vtkMRMLModelNode()
        modelNode.SetName(name)
        modelNode = cjyx.mrmlScene.AddNode(modelNode)
        modelNode.SetPolyDataConnection(sphere.GetOutputPort())
        modelNode.SetHideFromEditors(0)

        displayNode = cjyx.vtkMRMLModelDisplayNode()
        cjyx.mrmlScene.AddNode(displayNode)
