...
class MarkupsInViewsSelfTestLogic(ScriptedLoadableModuleLogic):
    ...
    def printViewAndSliceNodes(self):
        numViewNodes = cjyx.mrmlScene.GetNumberOfNodesByClass('vtkMRMLViewNode')
        print('Number of view nodes = ', numViewNodes)
        for vn in range(numViewNodes):
            viewNode = cjyx.mrmlScene.GetNthNodeByClass(vn, 'vtkMRMLViewNode')
            print('\t', viewNode.GetName(), "id =", viewNode.GetID())

        numSliceNodes = cjyx.mrmlScene.GetNumberOfNodesByClass('vtkMRMLSliceNode')
        print('Number of slice nodes = ', numSliceNodes)
        for sn in range(numSliceNodes):
            sliceNode = cjyx.mrmlScene.GetNthNodeByClass(sn, 'vtkMRMLSliceNode')
    ...
    def run(self):
        ...
        fidNode = cjyx.vtkMRMLMarkupsFiducialNode()
        cjyx.mrmlScene.AddNode(fidNode)
        ...
        self.nodeEvents = []
        observedEvents = [
            cjyx.vtkMRMLMarkupsNode.PointPositionDefinedEvent,
            cjyx.vtkMRMLMarkupsNode.PointPositionUndefinedEvent]
        ...
        assert(self.nodeEvents[0] == cjyx.vtkMRMLMarkupsNode.PointPositionDefinedEvent)
