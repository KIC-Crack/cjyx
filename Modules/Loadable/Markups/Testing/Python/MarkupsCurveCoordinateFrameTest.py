...
def createCoordinateSystemsModel(curve, axisLength=5):
    ...
    model = cjyx.mrmlScene.AddNewNodeByClass('vtkMRMLModelNode')
...
def addCoordinateSystemUpdater(updateInfo):
    model, curve, coordinateSystemAppender, curvePointToWorldTransform, transform, transformer = updateInfo
    observation = curve.AddObserver(cjyx.vtkMRMLMarkupsNode.PointModifiedEvent,
...
closedCurveNode = cjyx.mrmlScene.AddNewNodeByClass('vtkMRMLMarkupsClosedCurveNode')
pos = np.zeros(3)
...
centerlineCurve = cjyx.mrmlScene.AddNewNodeByClass('vtkMRMLMarkupsCurveNode')
...
import math
circleCurveNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsClosedCurveNode")
