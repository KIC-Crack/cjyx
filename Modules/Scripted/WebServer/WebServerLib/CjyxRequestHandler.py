...
class CjyxRequestHandler(object):
    ...
    def exec(self, request, requestBody):
        ...
        example:
    curl -X POST localhost:2016/cjyx/exec --data "cjyx.app.layoutManager().setLayout(cjyx.vtkMRMLLayoutNode.CjyxLayoutOneUpRedSliceView)"
        """
    ...
    def setupMRMLTracking(self):
        ...
        if not hasattr(self, "trackingDevice"):
            ...
            nodes = cjyx.mrmlScene.GetNodesByName('trackingDevice')
            if nodes.GetNumberOfItems() > 0:
                self.trackingDevice = nodes.GetItemAsObject(0)
                nodes = cjyx.mrmlScene.GetNodesByName('tracker')
                ...
                self.cube.Update()
                # display node
                self.modelDisplay = cjyx.vtkMRMLModelDisplayNode()
                self.modelDisplay.SetColor(1, 1, 0)  # yellow
                cjyx.mrmlScene.AddNode(self.modelDisplay)
                # self.modelDisplay.SetPolyData(self.cube.GetOutputPort())
                # Create model node
                self.trackingDevice = cjyx.vtkMRMLModelNode()
                self.trackingDevice.SetScene(cjyx.mrmlScene)
                ...
                cjyx.mrmlScene.AddNode(self.trackingDevice)
                # tracker
                self.tracker = cjyx.vtkMRMLLinearTransformNode()
                self.tracker.SetName('tracker')
                cjyx.mrmlScene.AddNode(self.tracker)
    ...
    def volumeSelection(self, request):
        ...
        currentIndex = 0
        if currentNodeID:
            nodes = cjyx.util.getNodes('vtkMRML*VolumeNode*')
    ...
    def volumes(self, request, requestBody):
        ...
        """
        volumes = []
        mrmlVolumes = cjyx.util.getNodes('vtkMRMLScalarVolumeNode*')
        mrmlVolumes.update(cjyx.util.getNodes('vtkMRMLLabelMapVolumeNode*'))
    ...
    def gridTransforms(self, request, requestBody):
        ...
        """
        gridTransforms = []
        mrmlGridTransforms = cjyx.util.getNodes('vtkMRMLGridTransformNode*')
    ...
    def postNRRD(self, volumeID, requestBody):
        ...
        except cjyx.util.MRMLNodeNotFoundException:
            node = None
        if not node:
            node = cjyx.vtkMRMLScalarVolumeNode()
            node.SetName(volumeID)
            cjyx.mrmlScene.AddNode(node)
...
""".replace("%%sizes%%", sizes).replace("%%directions%%", directions).replace("%%origin%%", origin)
    ...
    def fiducials(self, request, requestBody):
        ...
        """
        fiducials = {}
        for markupsNode in cjyx.util.getNodesByClass('vtkMRMLMarkupsFiducialNode'):
    ...
    def gui(self, request):
        ...
        :param viewersLayout: {fourup, oneup3d, ...} cjyx.vtkMRMLLayoutNode constants (CjyxLayout...View)
    ...
    def timeimage(self, request=''):
        ...
        cjyx.qMRMLUtils().qImageToVtkImageData(timeImage, vtkTimeImage)
