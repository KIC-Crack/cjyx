...
def downloadFromURL(uris=None, fileNames=None, nodeNames=None, checksums=None, loadFiles=None,
    ...
    See ``cjyx.mrmlScene.GetCacheManager().GetRemoteCacheDirectory()``
...
class SampleDataLogic:
    ...
    def downloadFileIntoCache(self, uri, name, checksum=None):
        ...
        destFolderPath = cjyx.mrmlScene.GetCacheManager().GetRemoteCacheDirectory()
            ...
            for attemptsCount in range(maximumAttemptsCount):
                ...
                if loadFileType == 'ZipFile':
                    if loadFile is False:
                        ...
                        break
                    outputDir = cjyx.mrmlScene.GetCacheManager().GetRemoteCacheDirectory() + "/" + os.path.splitext(os.path.basename(filePath))[0]
    ...
    def loadNode(self, uri, name, fileType='VolumeFile', fileProperties={}):
        ...
        for i in range(loadedNodes.GetNumberOfItems()):
            ...
            if not storageNode:
                continue
            cjyx.mrmlScene.RemoveNode(storageNode)
...
class SampleDataTest(ScriptedLoadableModuleTest):
    ...
    def test_downloadFromSource_downloadFiles(self):
        ...
        sceneMTime = cjyx.mrmlScene.GetMTime()
        ...
        self.assertEqual(sceneMTime, cjyx.mrmlScene.GetMTime())
    ...
    def test_downloadFromSource_loadMRBFile(self):
        ...
        self.assertTrue(os.path.exists(filePaths[0]))
        self.assertTrue(os.path.isfile(filePaths[0]))
        self.assertTrue(sceneMTime < cjyx.mrmlScene.GetMTime())

    def test_downloadFromSource_loadMRMLFile(self):
        logic = SampleDataLogic()
        tempFile = qt.QTemporaryFile(cjyx.app.temporaryPath + "/SampleDataTest-loadSceneFile-XXXXXX.mrml")
        tempFile.open()
        tempFile.write(textwrap.dedent("""
      <?xml version="1.0" encoding="UTF-8"?>
      <MRML  version="Cjyx4.4.0" userTags="">
      </MRML>
      """).strip())
        tempFile.close()
        sceneMTime = cjyx.mrmlScene.GetMTime()
    ...
    def test_downloadFromSource_loadNode(self):
        ...
        self.assertEqual(nodes[0], cjyx.mrmlScene.GetFirstNodeByName("MRHead"))
    ...
    def test_downloadFromSource_loadNodeFromMultipleFiles(self):
        ...
        self.assertEqual(len(nodes), 1)
        self.assertEqual(nodes[0], cjyx.mrmlScene.GetFirstNodeByName("DTIVolume"))
    ...
    def test_downloadFromSource_loadNodesWithLoadFileFalse(self):
        ...
        self.assertEqual(nodes[1], cjyx.mrmlScene.GetFirstNodeByName("CTChest"))
