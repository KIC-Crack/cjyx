...
class DICOMScalarVolumePluginClass(DICOMPlugin):
    ...
    def loadFilesWithSeriesReader(self, imageIOName, files, name, grayscale=True):
        ...
        name = cjyx.mrmlScene.GenerateUniqueName(name)
        if grayscale:
            volumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode", name)
        else:
            volumeNode = cjyx.mrmlScene.AddNewNodeByClass("vtkMRMLVectorVolumeNode", name)
        volumeNode.SetAndObserveImageData(imageChangeInformation.GetOutputDataObject(0))
        cjyx.vtkMRMLVolumeArchetypeStorageNode.SetMetaDataDictionaryFromReader(volumeNode, reader)
    ...
    def setVolumeNodeProperties(self, volumeNode, loadable):
        ...
        if volumeNode:
            ...
            if modality == "PT":
                displayNode = volumeNode.GetDisplayNode()
                if displayNode:
                    displayNode.SetAndObserveColorNodeID(cjyx.modules.colors.logic().GetPETColorNodeID(cjyx.vtkMRMLPETProceduralColorNode.PETheat))
    ...
    def export(self, exportables):
        for exportable in exportables:
            ...
            tags = {}
            tags['Patient Name'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMPatientNameTagName())
            tags['Patient ID'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMPatientIDTagName())
            tags['Patient Birth Date'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMPatientBirthDateTagName())
            tags['Patient Sex'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMPatientSexTagName())
            tags['Patient Comments'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMPatientCommentsTagName())
            tags['Study ID'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMStudyIDTagName())
            tags['Study Date'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMStudyDateTagName())
            tags['Study Time'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMStudyTimeTagName())
            tags['Study Description'] = exportable.tag(cjyx.vtkMRMLSubjectHierarchyConstants.GetDICOMStudyDescriptionTagName())
    ...
    class AcquisitionModeling:
        ...
        def gridTransformFromCorners(self, volumeNode, sourceCorners, targetCorners):
            ...
            gridTransform = cjyx.vtkMRMLGridTransformNode()
            gridTransform.SetName(cjyx.mrmlScene.GenerateUniqueName(volumeNode.GetName() + ' acquisition transform'))
            cjyx.mrmlScene.AddNode(gridTransform)
