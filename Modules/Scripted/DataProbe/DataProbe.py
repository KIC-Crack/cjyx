...
class DataProbeInfoWidget:
    ...
    def __init__(self, parent=None):
        ...
        self.CrosshairNode = cjyx.mrmlScene.GetFirstNodeByClass('vtkMRMLCrosshairNode')
        if self.CrosshairNode:
            self.CrosshairNodeObserverTag = self.CrosshairNode.AddObserver(cjyx.vtkMRMLCrosshairNode.CursorPositionModifiedEvent, self.processEvent)
    ...
    def processEvent(self, observee, event):
        ...
        # set image
        if (not cjyx.mrmlScene.IsBatchProcessing()) and sliceLogic and hasVolume and self.showImage:
        ...
        if hasattr(self.frame.parent(), 'text'):
            sceneName = cjyx.mrmlScene.GetURL()
            if sceneName != "":
    ...
    def generateViewDescription(self, xyz, ras, sliceNode, sliceLogic):
        ...
        if sliceNode.GetSliceSpacingMode() == cjyx.vtkMRMLSliceNode.PrescribedSliceSpacingMode:
            spacing = "(%s)" % spacing
    ...
    def _createMagnifiedPixmap(self, xyz, inputImageDataConnection, outputSize, crosshairColor, imageZoom=10):
        ...
        if (imin_trunc <= imax_trunc) and (jmin_trunc <= jmax_trunc):
            ...
            if vtkImageFromCanvas:
                qImage = qt.QImage()
                cjyx.qMRMLUtils().vtkImageDataToQImage(vtkImageFromCanvas, qImage)
